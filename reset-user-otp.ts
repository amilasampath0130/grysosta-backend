import mongoose from "mongoose";
import User from "./src/models/User.js";
import "dotenv/config";

const resetSpecificAdminOtp = async (email: string) => {
  try {
    await mongoose.connect(process.env.MONGO_URL!);

    const admin = await User.findOne({ email, role: "admin" });

    if (!admin) {
      console.log(`❌ Admin user with email ${email} not found`);
      process.exit(1);
    }

    // Reset OTP cooldown
    admin.adminOtp = undefined;
    admin.adminOtpExpires = undefined;
    admin.adminOtpSentAt = undefined;

    await admin.save();
    console.log(`✅ OTP cooldown reset for ${email}!`);
    console.log("You can now login immediately.");
  } catch (error) {
    console.error("❌ Error resetting admin OTP cooldown:", error);
  } finally {
    await mongoose.disconnect();
  }
};

// Reset for the user's email
resetSpecificAdminOtp("amilasampath0130@gmail.com");
